...
def get_route(ors_key, start, end):
    ...
    try:
        ...
        # Keep the raw 3D points so elevation can be derived if needed.
        try:
        ...
        pts = [(p[0], p[1]) for p in raw]  # (lat, lon) only, for the map

        # --- ascent/descent: ORS docs place these on the SEGMENT, not always
        # on the route itself, so check both. If neither is present, fall back
        # to computing them directly from the elevation values in the polyline.
        ascent_m = route.get("ascent")
        descent_m = route.get("descent")
        if ascent_m is None or descent_m is None:
            seg_asc = seg_desc = None
            for seg in route.get("segments", []):
                if "ascent" in seg or "descent" in seg:
                    seg_asc = seg.get("ascent")
                    seg_desc = seg.get("descent")
                    break
            ascent_m = ascent_m if ascent_m is not None else seg_asc
            descent_m = descent_m if descent_m is not None else seg_desc

        if (ascent_m is None or descent_m is None) and raw and len(raw[0]) >= 3:
            # last resort: sum elevation gains/losses from the 3D polyline
            calc_asc = calc_desc = 0.0
            for i in range(1, len(raw)):
                diff = raw[i][2] - raw[i - 1][2]
                if diff > 0:
                    calc_asc += diff
                else:
                    calc_desc += -diff
            ascent_m = ascent_m if ascent_m is not None else calc_asc
            descent_m = descent_m if descent_m is not None else calc_desc

        ascent_m = float(ascent_m or 0)
        descent_m = float(descent_m or 0)
