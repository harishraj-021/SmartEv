...
def clamp(v, lo, hi):
    """Keep a value within [lo, hi] so it never violates a widget's min/max."""
...
def get_route(ors_key, start, end):
    ...
    try:
        ...
        # ORS with elevation=True returns a 3D-encoded polyline (lat, lon, elevation).
        # IMPORTANT: the standard `polyline` PyPI package only decodes 2 dimensions -
        # feeding it a 3D string silently corrupts every coordinate (it misreads the
        # byte stream). So 2D lat/lon for the MAP always comes from a 2D-safe decode,
        # and elevation is never derived from a guessed polyline precision - only
        # trusted when ORS reports it explicitly as a number.
        try:
            pts_2d = polyline_lib.decode(route["geometry"], precision=5)  # [(lat, lon), ...]
        except Exception:
            pts_2d = []
        if not pts_2d:
        ...
        # ascent/descent: ORS's own docs place these on the SEGMENT, not always
        # on the route object, so check both - but never fabricate a value.
        ascent_m = route.get("ascent")
        descent_m = route.get("descent")
        if ascent_m is None or descent_m is None:
            for seg in route.get("segments", []):
                if "ascent" in seg or "descent" in seg:
                    ascent_m = ascent_m if ascent_m is not None else seg.get("ascent")
                    descent_m = descent_m if descent_m is not None else seg.get("descent")
                    break

        ascent_m = float(ascent_m) if isinstance(ascent_m, (int, float)) else 0.0
        descent_m = float(descent_m) if isinstance(descent_m, (int, float)) else 0.0

        # sanity bound: a realistic road can't climb more than ~8 m per km on average
        # over a long route (steepest real highways run 2-6 m/km); clamp absurd values
        # rather than let a bad response silently distort the range prediction.
        max_plausible = distance_km * 60.0  # generous ceiling, catches decode garbage
        ascent_m = min(ascent_m, max_plausible)
        descent_m = min(descent_m, max_plausible)
...
with st.expander("🌍 Step 1 — enter your trip (auto-fills terrain, weather & driving)", expanded=True):
    ...
    if st.button("🔎 Fetch route + weather", use_container_width=True):
        ...
        if not s or not e:
            ...
            if "error" not in w:
                st.session_state.v_temp = clamp(w["temp_c"], -20.0, 45.0)
                st.session_state.v_wind = clamp(w["wind_kmh"], 0.0, 120.0)

            if ors_key:
                route = get_route(ors_key, (s[0], s[1]), (e[0], e[1]))
                if "error" not in route:
                    st.session_state.v_distance = clamp(float(round(route["distance_km"], 1)), 1.0, 5000.0)
                    st.session_state.v_ascent = clamp(float(round(route["ascent_m"], 0)), 0.0, 8000.0)
                    st.session_state.v_descent = clamp(float(round(route["descent_m"], 0)), 0.0, 8000.0)
...
with right:
    ...
    distance = st.number_input("Trip distance (km)", 1.0, 5000.0, value=clamp(st.session_state.v_distance, 1.0, 5000.0))
    ascent = st.number_input("Total climb (m)  ← terrain", 0.0, 8000.0, value=clamp(st.session_state.v_ascent, 0.0, 8000.0))
    descent = st.number_input("Total descent (m)  ← regen", 0.0, 8000.0, value=clamp(st.session_state.v_descent, 0.0, 8000.0))
    temp = st.slider("Temperature (°C)  ← weather", -20.0, 45.0, value=clamp(st.session_state.v_temp, -20.0, 45.0))
    wind = st.slider("Wind speed (km/h)  ← weather", 0.0, 120.0, value=clamp(st.session_state.v_wind, 0.0, 120.0))
